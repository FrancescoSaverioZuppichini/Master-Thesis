...
\section{Robot}
We tested our framework on a legged crocodile-like robot called \emph{Krock} developed at \href{https://duckduckgo.com/?q=EPFL&atb=v154-1__&ia=web}{EPFL}.  Figure \ref{fig : krock}  shows the robot in real life with a coat and in a simulated environment.
...
The robot is $85$cm long and weights around $1.4$kg. Figure \ref{fig:krock-top} shows a top view of the robot highlighting each motor with a red marker.
\begin{figure}[htbp]
\centering
...
\end{figure}
Krock's moves by lifting and moving forward one leg after the other. We recorded some pictures of the robot's movements from the simulator and displayed each step in figure \ref{fig : krock-moving}. 
    \begin{figure}[htbp]
\centering
       ...
       \begin{subfigure}[b]{0.3\textwidth}
            ...
            \caption{$t=3$}
        \end{subfigure}    
    \caption{Krock moving forward. Each image represent one step in the movement.}
    \label{fig: krock-moving}
    \end{figure}
In our framework, we fixed the gait configuration to normal, showed in figure \ref{fig: krock}, where the body and the motors are at the same height. 
\section{Data Gathering}
\label{sec: data-gathering}
To train a traversability estimator in a supervised way we obviously need a dataset. This section describes in detail how to create, collect and process the data from the simulator. 
\subsection{Heightmap generation}
\label{subsec: heightmap-generation}
To collect the data through simulation we first need to generate meaningful terrain to be explored by the robot. Each map is represented by a heightmap, a 2D array, an image, where each pixel's value represents the terrain height. For example, figure \ref{fig : patch-example} shows a heightmap representing a real world quarry and the relative 3D rendered ground.
\begin{figure}[htbp]
    \centering
        \begin{subfigure}[b]{0.45\textwidth}
...
\label{fig : patch-example}
    \end{figure}
We created thirty maps of $513\times513$ pixel with a resolution of $0.02cm/pixel$ in order to represent a $10\times10$m terrain using 2D simplex noise \cite{simplex}. Simplex noise is a variant of Perlin noise \cite{perlin}, a widely used technique in the terrain generation litterature. 
We divided the maps into five main categories of terrains to cluster different ground features: $bumps$, $rails$, $steps$, $slopes$/$ramps$ and $holes$. Additionally, for some surfaces, we add three different rocky texture to create more complicated situations. All the maps configurations used in the simulator are listed in detail in table \ref{table: maps}. 
...
We used Webots to simulated Krock on the genereted terrain. The robot controlled was implemented by EPFL \todo{cite them?} and handed to IDSIA \todo{cite?}. The controller implements a ROS' node to publish \emph{Krock} status including its pose at a rate of $250hz$. We decide to reduce it to $50hz$ by using ROS build it \texttt{throttle} command. 
To load the map into the simulator, we first had to convert it to Webots's \emph{.wbt} file. Unfortunately, the simulator lacked support for heightmaps so we had to use a script to read the image and perform the conversion.

To communicate with the simulator, Webots exposes a wide number of ROS services, similar to HTTP endpoints, with which we can communicate. The client can use the services to get the value of a field of a Webots' Node, for example, if we want to get the terrain height, we have to ask for the field value \texttt{height} from \texttt{TERRAIN} node. In addition, to call one service, we first have to get the correct type of message we wish to send and then we can call it. We decided to develop a little library called \href{https://github.com/FrancescoSaverioZuppichini/Master-Thesis/tree/master/core/utilities/webots2ros}{\texttt{webots2ros}} to hide all the complexity needed to fetch a field value from a node.

We also implemented one additional library called \href{https://github.com/FrancescoSaverioZuppichini/Master-Thesis/tree/master/core/simulation/agent}{\texttt{Agent}} to create reusable robot's interfaces independent from the simulator. The package supports callbacks that can be attached to each agent adding additional features such as storing its interaction. Finally, we used \emph{Gym} \cite{gym}, a toolkit to develop and evaluate reinforcement learning algorithms, to define our environment. Due to the library's popularity, the code can easily be shared with other researches or we may directly experiment with already made RL algorithm in the future without changing the underlying infrastructure.

\subsection{Real world maps}
To later evaluate the model's performance, we decide to use real-world terrains. We gather two heightmaps produced by ground mapping flying drones from \href{https://www.sensefly.com/education/datasets/}{sensefly}'s dataset, a quarry, and a small village, their original location and 3D render is presented in figure \ref{fig : real-maps} .
...
\end{figure}
\section{Simulation}
To collected \emph{Krock}'s interaction with the environment, we spawned the robot on the ground and let it move going forward with a fixed controller for $t$ seconds. We repeated this process $n$ times per  map.
    Unfortunately, spawning the robot was not a trivial task. In certain maps, for instance, \emph{bars1}, a map with tons of walls, we had to avoid spawning Krock on an obstacle. Otherwise, the run will be ruined by \emph{Krock} getting stuck at the beginning introducing noise in the dataset. To solve the problem, we defined two spawn strategies, a random spawn, and a flat ground spawn strategy. The first one is employed in most of the maps without big obstacles such as \emph{slope\_rocks}. This strategy just spawns the robot in a random position and rotation. 
On maps with obstacles, we adopted the flat ground strategy. This method selects suitable spawn positions by using a sliding window on the heightmap of size equal Krock's footprint and check if the mean pixel value is lower than a small threshold. If so, we stored the center coordinates of the patch as a candidate spawning point. Intuitively, if a patch is flat then its mean value will be close to zero.
Since there may be more flat spawning positions than simulations needed, we had to reduce the size of the candidate points. To maintain the correct distribution on the map to avoid generating the robot always in the same cloud of points, we used K-Means with $k$ clusters where $k$ was equal to the number of simulations we wished to run. By clustering, we guaranteed to cover all region of the map removing any bias.  Picture \ref{fig : spawn-strat} illustrates this strategy on \emph{bars1}. 
\begin{figure}[htbp]
    \begin{subfigure}[b]{0.45\textwidth}
...
After run all the simulations, we had to extract the patches for each stored pose $p_i$ and compute the advancement for a given time window, $\Delta t$.
\subsection{Parse stored data}
First, we turned each \emph{.bag} file into a pandas dataframe and cache them into \emph{.csv} files. We used \href{https://github.com/aktaylor08/RosbagPandas}{\texttt{rosbag\_pandas}}, an open source library we ported to python3, to perform the conversion.
Then, we loaded the data frames with the respective heightmaps and start the data cleaning process. We remove the rows corresponding to the first second of the simulation time to account for the robot spawning time. Then we eliminate all the entries where the \emph{Krock} pose was near the edges of a map, we used a threshold of $22$ pixels since we notice  \emph{Krock} getting stuck in the borders of the terrain during a simulation. 
...
Before caching again the resulting data frames into \emph{.csv} files, we convert the robot's position into heightmap's coordinates that are used later to crop the correct region of the map.

To computed the robot's advancement in a time window $\Delta t$ we looked for each stored pose ,$p_t$, in the future, $p_{t + \Delta t}$, \ref{fig :  patch-extraction}/

The correct value of $\Delta t$ is crucial. We want a time window small enough to avoid smoothing too much the advancement and making obstacle traversable, and big enough to include the full legs motion. We empirically set $\Delta t = 2$ since it allows Krock to move both its legs and does not flat the advancement too much. 
\subsection{Extract patches}
Each patch must contain both Krock's footprint, to include the situations where the obstacle is under the robot and a certain amount of ground region in front of it. Intuitively, we want to include in each patch the correct amount of future information according to the selected time window. Thus, we should add the maximum possible ground that Krock could traverse. 

To find out the correct value, we must compute the maximum advancement on flat ground for the $\Delta t$ and use it to calculate the final size of the patch. We compute it by running some simulations of \emph{Krock} on flat ground and averaging the advancement getting a value of $71$cm in our $\Delta t = 2$s.
...
Due to its characteristic locomotion, Krock tries to overcome the obstacle using its legs to move itself to the top but it fell backs producing a irregular advancement where first it is positive and then negative. Figure \ref{fig : krock-bumps-advs} shows the advancements on this map with $\Delta t = 2$, we can notice the noisy output.
...
\end{figure}
A wheeled robot, on the other hand, will not produce such a graph since it cannot free itself easily from an obstacle. Image a wheeled robot moving forward, once it hits an obstacle it stops moving, if we plot its advancement in this situation it will look more like a step than a series of spikes.

\paragraph{Ramps}
The threshold should be small enough to not create any false positive, patches that are traversable but were classified as not. So, we let the robot walk up hill on the $slope_rocks1$ map with a height scaling factor set to $5$. We knew from empirical experiments that the robot is able to climb this steep ramp, the advancement is showed in figure \ref{fig : krock-ramps-sim}.
\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Vanilla model proposed by Chavez-Garcia et al. \cite{omar2018traversability}. The rectangle represents the building block of each layer. Convolution layers are described by the kernel size, the number of filters and the stride.}
    \label{fig : omar}
\end{figure}
\subsection{ResNet}
...
The next figure visualizes the SE module.
...
\begin{figure}[htbp]
    ...
    \end{subfigure} 
    \caption{MicroResNet with initial features size of $16$ and stride of $2$ with and without the Squeeze and Excitation module. The rectangle represents the building block of each layer. Convolutional layers are described by the kernel size, the number of filters and the stride. Lines and dashed lines between layers represent the residual operations and the shortcut respectively.}
...
Similar to drop out, it sets to zero random regions of pixels with defined boundaries. 
...
\subsection{Numpy}
\href{https://www.numpy.org/}{Numpy} is a fundamental package for any scientific use. It allows expressing efficiently any matrix operation using its broadcasting functions. Numpy is used across the whole pipeline to manipulate matrices.
...
\subsection{Pandas}
To process the data from the simulations we rely on \href{https://pandas.pydata.org/}{Pandas}, a Python library providing fast, flexible, and expressive data structures in a tabular form. Pandas is well suited for many different kinds of data such as handle tabular data with heterogeneously-typed columns, similar to SQL table or Excel spreadsheet, time series and matrices. We take advantages of the relational data structure to perform custom manipulation on the rows by removing the outliers and computing the advancement.

Generally, pandas do not scale well and it is mostly used to handle small dataset while relegating \"big data\" to other frameworks such as Spark or Hadoop. We used Pandas to store the results from the simulator and inside a Thread Queue to parse each \emph{.csv} file efficiently. 
...
\subsection{Mayavi}
Mayavi is a scientific data visualization and plotting in python library. We adopt it to render in 3D all the surfaces used in our framework.
\end{document}