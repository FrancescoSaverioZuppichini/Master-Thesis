...
\end{figure}
To evaluate the model's prediction, we run all the patches several time on the simulator and average the results. We highlighted the false positive and the false negative by red and blu respectively. The model's fails to predicts some of the edge cases. The false positive are the patches with a wall just ahead Krock of heights between \todo{todo}, while the false negative are the inputs at $20$cm. The overall accuracy is \todo{todo}. Even if the model wrongly classified some of the inputs, all those errors are in the edge cases where the predicted classes' probability is not maximum. So, even in those cases the model correctly shown uncertainty. Also, the prediction changes smoothly without any spikes accordendly to the features of the terrain. For example, if a wall was previous classified as untraversable, all the taller walls in the same position are classified with the same class.

\todo[inline]{missing ground truth heatmap}

Since we spawned the robot directly on the patch inside the simulator, the outputs may change across different runs. Sometimes, two runs on the same patch can produce different advancement due to some really small changes in the initial position of the robot caused the spawing procedure or some lag. So, to completness, we also shows the variance between all simulation's runs to highlighted cases where the advancement changes the most across different runs. 
\todo[inline]{FINISH}
