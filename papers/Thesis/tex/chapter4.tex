...
This is a hard map to traverse due to the high number of walls, almost any wall can not be overcome by krock. Interesting, we can identify a tunnel near the bottom center of the maps that shows how the model correctly label those patches depending on the orientation.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
      ...
      % \label{fig: bars-r2l}
  \end{subfigure}
  \caption{Detail of a region in the bars map where there are two walls forming a tunnel.}
  \end{figure}

\subsubsection{Sullen}
The following map is a small village downloaded from sensefly online \href{https://www.sensefly.com/education/datasets/?dataset=1420}{datasets hub}. Since the map is huge, we crop a 
\end{document}